...
class ModelVietnamse:
    ...
    def __init__(self, charList, restore=False):
        ...
        # input images
        self.inputImgs = tf.placeholder(tf.float32, shape=(None, ModelVietnamse.imgSize[0], ModelVietnamse.imgSize[1]))
    ...
    def trainBatch(self, minibatch):
        ...
        feed = {self.inputImgs : minibatch.imgs, self.gtTexts : sparse , self.seqLen : [ModelVietnamse.maxTextLen] * numBatchElements, self.learningRate : lr, self.keepprob: 0.75, self.is_train: True}
    ...
    def inferBatch(self, minibatch):
        ''' recognize the texts '''
        numBatchElements = len(minibatch.imgs)
        feed = {self.inputImgs : minibatch.imgs, self.seqLen : [ModelVietnamse.maxTextLen] * numBatchElements, self.keepprob: 1.0, self.is_train: False}
    ...
    def save(self):
        ''' save model to file '''
        self.ID += 1
        self.saver.save(self.sess, '../model_vietnamese/crnn-model', global_step=self.ID)