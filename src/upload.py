...
from main import infer_by_web, infer_by_web_vietnamese
...
@app.route("/upload_page_vietnamese", methods=["GET"])
def upload_page_vietnamese():
    return render_template("upload_vietnamese.html")
...
@app.route("/upload_vietnamese", methods=["POST"])
def upload_vietnamese():
    # folder_name = request.form['uploads']
    target = os.path.join(APP_ROOT, 'static/')
    print(target)
    if not os.path.isdir(target):
    ...
    for upload in request.files.getlist("file"):
        ...
        print("Save it to:", destination)
        upload.save(destination)
        result = predict_image_vietnamese(destination, option)
        print("Prediction: ", result)
    # return send_from_directory("images", filename, as_attachment=True)
    return render_template("complete.html", image_name=savefname, result=result)

def predict_image_vietnamese(path, type):
    print(path)
    return infer_by_web_vietnamese(path, type)
